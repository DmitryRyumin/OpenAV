...
class MessagesTestAudioVisual(Run):
    ...
    def __post_init__(self):
        ...
        self._description: str = self._("Автоматическое тестирование на аудиовизуальных данных")
...
# ######################################################################################################################
@dataclass
class RunTestAudioVisual(MessagesTestAudioVisual):
    """Класс для автоматического тестирования на аудиовизуальных данных"""
    ...
    def _valid_yaml_config(self, config: Dict[str, Union[str, bool, int, float]], out: bool = True) -> bool:
        ...
        for key, val in config.items():
            ...
            if key == "subfolders":
                ...
                # Проверка значения
                if type(val) is not dict or len(val) == 0 or not all(subfolder in val for subfolder in SUBFOLDERS_TEST):
                    continue
            ...
            if key == "n_classes" or key == "hidden_units" or key == "hidden_features" or key == "input_dim":
                # Проверка значения
    ...
    def run(self, metadata: ModuleType = openav, resources: ModuleType = rsrs, out: bool = True) -> bool:
        """Запуск автоматического тестирования на аудиовизуальных данных
        ...
        Returns:
             bool: **True** если автоматическое тестирование на аудиовизуальных данных произведено успешно,
...
def main():
    # Запуск автоматического тестирования на аудиовизуальных данных
    test_av = RunTestAudioVisual(lang="ru", path_to_logs="./openav/logs")
    test_av.run(out=True)
