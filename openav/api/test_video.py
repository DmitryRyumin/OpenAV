...
"""Автоматическое тестирование на  видеоданных
"""
...
from openav.modules.lab.video import METRICS_VIDEO
...
class MessagesTestVideo(Run):
    ...
    def __post_init__(self):
        ...
        self._description: str = self._("Автоматическое тестирование на видеоданных")
...
# ######################################################################################################################
@dataclass
class RunTestVideo(MessagesTestVideo):
    """Класс для автоматического тестирования на видеоданных"""
    ...
    def _valid_yaml_config(self, config: Dict[str, Union[str, bool, int, float]], out: bool = True) -> bool:
        ...
        for key, val in config.items():
            ...
            if key == "metric":
                # Проверка значения
                if type(val) is not str or (val in METRICS_VIDEO) is False:
                    continue
    ...
    def _load_config_yaml(self, resources: ModuleType = rsrs, config="test_video.yaml", out: bool = True) -> bool:
...
def main():
    # Запуск процесса тестирования
    vad = RunTestVideo(lang="ru", path_to_logs="./openav/logs")
    vad.run(out=True)
