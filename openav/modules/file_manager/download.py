...
class Download(DownloadMessages):
    ...
    def download_file_from_url(self, url: str, force_reload: bool = True, out: bool = True) -> int:
        ...
        try:
            ...
            self.inv_args(__class__.__name__, self.download_file_from_url.__name__, out = out); return 400
        else:
            try:
            ...
            ): self.message_error(self._could_not_process_url, out = out); return 404
            except Exception: self.message_error(self._unknown_err, out = out); return 404
            else:
                # Имя файла
                ...
                try:
                    ...
                    self.message_error(self._url_incorrect, out = out); return 404
                except Exception: self.message_error(self._unknown_err, out = out); return 404
                else:
                    ...
                    if self.create_folder(self.path_to_save_models, out = False) is False: return 404
                    ...
                    except Exception: self.message_error(self._unknown_err, space = self._space, out = out); return 404
                    else:
                        ...
                        if Path(local_file).is_file() is True and force_reload is False:
                            ...
                            if r.status_code == 200:
                                ...
                                try:
                                    ...
                                    self.message_error(
                                        self._url_incorrect_content_length, space = self._space, out = out
                                    ); return 404
                                else:
