...
class FileManager(FileManagerMessages):
    ...
    def get_paths(self, path: Iterable, depth: int = 1, out: bool = True) -> List[Optional[str]]:
        ...
        try:
        ...
        except TypeError: self.inv_args(__class__.__name__, self.get_paths.__name__, out = out); return []
        else:
            ...
            if depth != 1 and len(new_path) > 0: return self.get_paths(new_path, depth - 1)
    ...
    def search_file(self, path_to_file: str, ext: str, create: bool = False, out: bool = True) -> bool:
        ...
        # Проверка аргументов
        if (type(path_to_file) is not str or type(ext) is not str or not ext or type(create) is not bool
                or type(out) is not bool):
            self.inv_args(__class__.__name__, self.search_file.__name__, out = out); return False

        # Файл не передан
        if not path_to_file: self.message_error(self._file_name.format(ext.lower()), out = out); return False
        ...
        if ext != extension.replace('.', ''):
            self.message_error(self._wrong_extension.format(ext), space = self._space, out = out); return False
        ...
        if os.path.isfile(path_to_file) is False:
            ...
            self.message_error(
                self._file_not_found.format(os.path.basename(path_to_file)), space = self._space, out = out
            )
            return False
    ...
    def search_files(
        ...
        # Проверка аргументов
        if (type(path_to_folder) is not str or type(exts) is not list or len(exts) == 0
                or type(sort) is not bool or type(out) is not bool):
            self.inv_args(__class__.__name__, self.search_files.__name__, out = out); return []

        path_to_folder = os.path.normpath(path_to_folder)
        exts = [ext.replace('.', '') for ext in exts]

        # Директория не передана
        if os.path.isdir(path_to_folder) is False: self.message_error(self._dir_name, out = out); return []
        ...
        if len(files) == 0: self.message_error(self._files_not_found, space = self._space, out = out); return []

        # Сортировка файлов
        if sort is True: return sorted(files)
    ...
    def create_folder(self, path_to_folder: str, out: bool = True) -> bool:
        ...
        if type(path_to_folder) is not str or not path_to_folder or type(out) is not bool:
            self.inv_args(__class__.__name__, self.create_folder.__name__, out = out); return False
        ...
        if not os.path.exists(path_to_folder):
            ...
            try: os.makedirs(path_to_folder)
            except Exception:
                self.message_error(self._folder_not_create.format(path_to_folder), space = 4, out = out); return False
            else: return True
        else: return True
    ...
    def clear_folder(self, path_to_folder: str, out: bool = True) -> bool:
        ...
        # Проверка аргументов
        if type(path_to_folder) is not str or not path_to_folder or type(out) is not bool:
            self.inv_args(__class__.__name__, self.clear_folder.__name__, out = out); return False
        ...
        else: self.message_error(self._clear_folder_not_found.format(path_to_folder), out = out); return False
