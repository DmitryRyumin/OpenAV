...
class AVDataset(Dataset):
    def __init__(
        ...
        self.pl = [2048, 64, None, True, "reflect", 2.0, 64, "slaney", True, None]  # для извлечения спектрограмм
    ...
    def get_metadata(self, name_file):
        ...
        a_fss = []  # аудио признаки
        v_fss = []  # видео признаки
        lip_areas = []
        ...
        w = int(video.get(cv2.CAP_PROP_FRAME_WIDTH))  # получаем информацио о ширине кадра
        h = int(video.get(cv2.CAP_PROP_FRAME_HEIGHT))  # получаем информацио о высоте кадра
        total_frames = int(video.get(cv2.CAP_PROP_FRAME_COUNT))  # общее кол-во кадров
        ...
        if math.modf(len_spec)[0] == 0:
            len_spec += 1
        len_spec = (
            math.ceil(len_spec) + 1
        )  # для фиксирования ширены спектрограммы (тут может быть косяк надо проверить)

        # работа с аудио сигналом
        try:
            wav = self.convert_video_to_audio(name_file)
            # print(len(wav), self.sampling_rate)
            # делим аудиосигнал на сегменты
            for i in range(0, len(wav) + 1, len_audio):
                # print(i, len_spec)
                curr_wav = wav[i : min(i + len_audio, len(wav))]
                if len(curr_wav) > self.pl[0]:
                    # извлекаем фичи для сегмента
                    ...
                    a_fss.append(db_m_s)
                # прерываем если сегмент выходит за максимальную длину сигнала
